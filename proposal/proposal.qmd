...
The [R Contribution Working Group (RCWG)](https://contributor.r-project.org/working-group) has been involved in a number of projects to foster a wider, more diverse, community of contributors to base R. The group facilitates active contribution by the community, especially through the R Dev Days. This proposal is specifically related to efforts to support contributions to the translations of messages in base R. The aim is to improve the technical infrastructure of the translations process by providing a common glossary that can be used by all translation projects in the R ecosystem, sharing the translations across projects to avoid duplicated work.
...
- Documented process for people to suggest additions to and deletions from the common glossary.
  - Preferred: pull request on the underlying CSV so that it is easy to update the common glossary once proposed changes are reviewed. Another benefit of having the CSV format is the glossary can be re-used by people without independently of the proposed package 
